...
const FloatingNode = ({ label, x, y, delay = 0, icon }: NodeProps) => (
  <motion.div
    className="absolute glass-panel px-4 py-2 flex items-center gap-2"
    style={{ left: `${x}%`, top: `${y}%` }}
...
export const FloatingNodes = () => {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      ...
      <FloatingNode label="Boomi" x={75} y={15} delay={0.4} />
      <FloatingNode label="SAP S/4 HANA" x={10} y={60} delay={0.6} />
      <FloatingNode label="SAP API Gateway" x={80} y={55} delay={0.8} />
      <FloatingNode label="Advanced Event Mesh" x={5} y={85} delay={1.0} />
      <FloatingNode label="MuleSoft" x={75} y={80} delay={1.2} />
    </div>
  );
};
